...
- **已處理**：10 則已於 `d78d82f` 修復（6 P1 + 4 P2）。
- **待處理**：P1 × 6、P2 × 27、P3 × 4。
- **誤報/過時**：2 則（無需處理）。
...
| # | 位置 | 問題 | 建議修法 |
| --- | --- | --- | --- |
| 1 | `store.ts` | 併發 OAuth/connect 可能產生多列，破壞單列不變式 | DB 層 unique constraint + conflict-safe upsert（目前僅 app 層保證） |
| 24 | `fuzzy.ts` | 「Meeting Budget」會誤配到「Meeting Notes」：只要第一個字對上就給 leading 分數 | 重疊的 leading words 需**全部**比對才給分；允許 query 尾端多字 |
| 25 | `MessageInputActions/Notion.tsx` | Clear 按鈕預設 `type="submit"`，點擊可能送出表單 | 加 `type="button"` |
| 26 | `api/chat/route.ts` | 信任 caller 送來的 page IDs，可把未授權頁面存進 `notion_pages` | server 端對 connected workspace 驗證 id 才存 |
| 40 | `actions/notion/read.ts` | agent tools 可讀 token 能及的任何頁面，繞過 per-conversation scope（ADR-0001） | 讀取前驗證 pageId 屬於對話選取/`notion_search` 解析結果 |
| 41 | `prompts/search/researcher.ts` | 頁面 title 直接插入 system prompt，可能被當成指令（prompt injection） | 將 `type`/`id`/`title` 當 untrusted metadata escape/序列化 |
...
**Still open (next round / PR2):**
- P1: enforce the one-row invariant at the DB level (unique constraint);
  require ALL overlapping leading title words in fuzzy matching; fix the
  Clear button submitting the form; validate caller-supplied page ids
  server-side; enforce the per-conversation page scope inside the agent
  tools; escape page titles before inserting them into the researcher
  prompt.
...
- 待辦清單 → 下一輪 review-fix commit（P1 優先），其餘 P2 併入 PR2 或後續 PR
